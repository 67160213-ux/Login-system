...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (strlen($password) < 8) $errors[] = "รหัสผ่านต้องยาวอย่างน้อย 8 ตัวอักษร";
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) $errors[] = "อีเมลไม่ถูกต้อง";
    if ($display_name === "" || mb_strlen($display_name) > 100) $errors[] = "กรุณากรอกชื่อ–นามสกุล (ไม่เกิน 100 ตัวอักษร)";
    ...
    if (!$errors) {
        ...
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows > 0) $errors[] = "Email นี้ถูกใช้แล้ว";
        $stmt->close();
    }
...
<div class="container">
<h1>สมัครสมาชิก</h1>
<?php if ($errors): ?><div class="alert error"><?php foreach ($errors as $m) echo "<div>".e($m)."</div>"; ?></div><?php endif; ?>
...
<p style="text-align:center; margin-top:16px;">มีบัญชีแล้ว? <a href="login.php">เข้าสู่ระบบ</a></p>
</div>
</body>
</html>
